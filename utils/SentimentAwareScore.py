...
class SentimentAwareScorer(BM25F):
    def __init__(self, fieldname="text", k1=1.5, b=0.75, positive_boost=1.5, negative_boost=0.5, neutral_boost=1.0):
        super().__init__(k1, b)
...
class SentimentAwareBM25FScorer(BM25FScorer):
    ...
    def score(self, matcher):
        ...
        positive_sentiment = float(doc.get("positive_sentiment", 0.0))
        negative_sentiment =  float(doc.get("negative_sentiment", 0.0))
        neutral_sentiment =  float(doc.get("neutral_sentiment", 0.0))
